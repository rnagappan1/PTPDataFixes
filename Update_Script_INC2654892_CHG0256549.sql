...
#  | -------------      ----------    ---------------------------------------------------------------
#  | N.Ramesh       13-Nov-2025     Created for : INC2654892
-------------------------------------------------------------------------------------------------*/
SET SERVEROUTPUT ON ;
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    xxgil_oims_comm_utils_pkg.log_audit_event ( p_stg_hdr_id => i.STG_HDR_ID, p_stg_line_id => NULL, p_audit_event_type => 'INV_STATUS_CHANGE', p_new_value => 'EBS-OK-TO-PAY', p_old_value => i.invoice_status, p_manaully_release => NULL, p_event_owner_id => NULL, p_exception_id => NULL, p_comments => 'Updated status through Data fix for : INC2654892', x_return_status => lv_return_status, x_return_msg => lv_return_msg );
